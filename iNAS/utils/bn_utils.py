...
@torch.no_grad()
def update_bn_stats(model, data_loader, num_iters: int = 200, sample_cfg=None, progress=True):
    """
    ...
    dl_iterator = tqdm(data_loader) if progress is True else data_loader

    for ind, inputs in enumerate(dl_iterator):
        inputs['cfg'] = sample_cfg
        ...
        if ind == num_iters - 1:
            break
    assert ind == num_iters - 1, ('update_bn_stats is meant to run for {} iterations, '
