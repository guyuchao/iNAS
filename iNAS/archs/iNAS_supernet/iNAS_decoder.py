...
from iNAS.archs.iNAS_supernet.iNAS_util import DynamicConv2d, DynamicDecoderFusion, DynamicDecoderUpsample, Identity
...
class DynamicDecoder(nn.Module):
    ...
    def __init__(self, cfg, level_channel_list, num_classes, activation, output_level_ids):
        ...
        for level_name, level_cfg in cfg.items():
            ...
            self.mainops[level_name] = DynamicDecoderFusion(
                max_in_channels=channels, max_kernel_size_array=max_kernel_size_array)
        ...
        for level_name, level_cfg in cfg.items():
            ...
            for connect_idx in connect_list:
                if level_idx == connect_idx:
                    ...
                    self.branches[level_name][str(connect_idx)] = DynamicDecoderUpsample(
        ...
        for level_name, channels in level_channel_list.items():
            level_idx = int(level_name.split('_')[1])
            if level_idx in self.output_level_ids:
                self.decoder_dict[level_name] = nn.Sequential(
                    OrderedDict([
                        ('conv', DynamicConv2d(channels, num_classes, kernel_size=1)),
                        ('act', act_class),
        ...
        for level_name in decoder_order:
            ...
            for connect_idx in connection_list:
                up_feature = self.branches[level_name][str(connect_idx)](x['level_%d' % connect_idx],
                                                                         level_channels_list[level_name])
                cell_features.append(up_feature)

            cell_op = self.mainops[level_name]
            cell_features = cell_op(cell_features, cfg[level_name]['kernel'])
